...
<div class="max-w-2xl bg-white rounded-lg shadow p-6">
    <form method="POST" action="/dashboard/indikator-kinerja/{{ $indikator->indikator_id }}">
        @csrf
        @method('PUT')
        <div class="mb-4">
            ...
            <select name="kriteria_id" class="w-full px-4 py-2 border border-gray-300 rounded-lg" required>
                ...
                <option value="{{ $item->kriteria_id }}" {{ $indikator->kriteria_id == $item->kriteria_id ? 'selected' : '' }}>{{ $item->kode_kriteria }} - {{ $item->nama_kriteria }}</option>
                @endforeach
            </select>
        </div>
        ...
        <div class="mb-4">
            ...
            <input type="text" name="kode_indikator" value="{{ $indikator->kode_indikator }}" class="w-full px-4 py-2 border border-gray-300 rounded-lg" required>
        </div>

        <div class="mb-4">
            <label class="block text-gray-700 text-sm font-medium mb-2">Nama Indikator</label>
            <input type="text" name="nama_indikator" value="{{ $indikator->nama_indikator }}" class="w-full px-4 py-2 border border-gray-300 rounded-lg" required>
        </div>
            ...
            <div id="target-container">
                ...
                @if(is_array($targets))
                    ...
                    <div class="flex gap-2 mb-2">
                        <input type="text" name="target[]" value="{{ $target }}" class="flex-1 px-4 py-2 border border-gray-300 rounded-lg" required>
                        <button type="button" onclick="removeTarget(this)" class="bg-red-500 text-white px-4 py-2 rounded-lg hover:bg-red-600 {{ $index == 0 ? 'hidden' : '' }}">Hapus</button>
                    </div>
                    @endforeach
                @else
                    <div class="flex gap-2 mb-2">
                        <input type="text" name="target[]" value="{{ $indikator->target }}" class="flex-1 px-4 py-2 border border-gray-300 rounded-lg" required>
