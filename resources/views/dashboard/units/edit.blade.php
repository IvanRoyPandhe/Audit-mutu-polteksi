...
<div class="max-w-2xl bg-white rounded-lg shadow p-6">
    <form method="POST" action="/dashboard/units/{{ $unit->unit_id }}">
        @csrf
        @method('PUT')
        <div class="mb-4">
            <label class="block text-gray-700 text-sm font-medium mb-2">Nama Unit</label>
            <input type="text" name="nama_unit" value="{{ $unit->nama_unit }}" class="w-full px-4 py-2 border border-gray-300 rounded-lg" required>
        </div>

        <div class="mb-4">
            <label class="block text-gray-700 text-sm font-medium mb-2">Tipe Unit</label>
            <input type="text" name="tipe_unit" value="{{ $unit->tipe_unit }}" class="w-full px-4 py-2 border border-gray-300 rounded-lg" required>
        </div>

        <div class="mb-6">
            <label class="block text-gray-700 text-sm font-medium mb-2">Pimpinan</label>
            <input type="text" name="pimpinan" value="{{ $unit->pimpinan }}" class="w-full px-4 py-2 border border-gray-300 rounded-lg" required>
        </div>
